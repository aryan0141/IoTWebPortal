      ...
      dataArray.forEach((c) => {
        ...
        sensorDetailArray.forEach((currSensorDetail) => {
          ...
          if (
            ...
            imageId &&
            currSensorDetail &&
            currSensorDetail.isVerified == false
          ) {
...
function createDiv(user, geolocation, imageId, currSensorDetail) {
  ...
  var sensorVerificationCards = document.getElementById(
    "sensorVerificationCards"
  );
...
async function getAllUnusedSensors() {
  const settings = {
    method: "GET",
    headers: {
      Accept: "application/json",
  ...
  document.getElementById("unusedSensorsLoader").style.display = "block";
  document.getElementById("unusedSensorsCardDiv").style.display = "none";
  const response = await fetch("/getAllUnusedSensors", settings);
  const allUnusedSensorsData = await response.json();
  console.log(allUnusedSensorsData, "Here");

  setTimeout(function () {
    extractUnusedSensors(allUnusedSensorsData);
  }, 50);
  document.getElementById("unusedSensorsLoader").style.display = "none";
  document.getElementById("unusedSensorsCardDiv").style.display = "block";
}

function extractUnusedSensors(allUnusedSensorsData) {
  let mainCardDiv = document.getElementById("unusedSensorsCardDiv");
  var anyUnusedSensorFound = false;
  // Removing the previous data:
  while (mainCardDiv.firstChild) {
    mainCardDiv.removeChild(mainCardDiv.lastChild);
  }
  allUnusedSensorsData.map((data) => {
    if (data.isUsed == false) {
      var cardDiv = document.createElement("div");
      cardDiv.className = "unusedSensorCard";
      cardDiv.innerHTML = `
                                  <p>
                                  <b>Sensor Id:</b> ${data.sensorId}
                                  </p>
                                  <p>
                                  <b>Microcontroller Id:</b> ${data.microControllerID}
                                  </p>
                                `;
      mainCardDiv.append(cardDiv);
      anyUnusedSensorFound = true;
    }
  });
  if (anyUnusedSensorFound == false) {
    document.getElementById("noUnusedSensorText").style.display = "block";
  } else {
    document.getElementById("noUnusedSensorText").style.display = "none";
  }
}

getAllUnusedSensors();
setInterval(getAllUnusedSensors, 4000);